Problem: https://lightoj.com/problem/help-hanzo
...
{
	ll range = b - a;
	for(int i = 0; i < b - a + 1; i++) flag[i] = false;
	
	if(a < 2) a = 2;
	
	for(int i = 0; i < primes.size() and (ll)(primes[i] * primes[i]) <= b; i++) {
	
		ll _a = (a / primes[i]) * primes[i];
		if(_a < a) _a += primes[i];
		
		if(_a < (ll)(primes[i] + primes[i])) _a = primes[i] + primes[i];
		
		for( ; _a <= b; _a += primes[i]) {
			flag[_a - a] = true;
		}
	}
	int ans = 0;
